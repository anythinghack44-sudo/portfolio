...
export function Capabilities() {
  ...
  return (
    <section className="overflow-hidden py-20 lg:py-28">
      ...
      <div className="relative mb-16 overflow-hidden lg:mb-20">
        <div ref={marqueeRef} className="flex whitespace-nowrap will-change-transform">
          {/* Render 3 sets to ensure seamless loop */}
          {[...Array(3)].map((_, setIndex) => (
            <div key={setIndex} className="flex shrink-0 items-center gap-8">
      ...
      <div className="gutter grid gap-10 lg:grid-cols-3 lg:gap-6">
        {capabilities.pillars.map((pillar) => (
          <div
            key={pillar.index}
            className="border-t border-border pt-8 lg:pt-10"
          >
            <p className="type-label mb-4 text-accent">{pillar.index}</p>
            <h3 className="type-subhead mb-4 text-foreground">{pillar.title}</h3>
